...
const HOST_PLACEHOLDER_IMAGE = 'https://res.cloudinary.com/wajooba/image/upload/v1697785936/master/user-image-placehoder.jpg';
...
const MAP_PREVIEW_IMAGE = 'https://lh3.googleusercontent.com/aida-public/AB6AXuCrTJWlma4Wfv-9Meg_ZzuxJjlBYv7go16lnaBD-_eAphPXvnYb3Gg4KPp2Kck5ASS2JAHqC9F0fval2_TQ_H9ToYFX8yRKwTaDPZvJYMDzyy70CVvdS25Lzke57ncAot6_n6jQYEyJiS5pp3GkoDh8bnNrY5AShEexUY6PJXJcwG3ugM26zl9L4vezusrABGF7FUvG6sNPuVfcd78my71hqwPjcYiqKVdxmh7gbVOMob_gpiVvycZbFLIwtcQOlUb05vcpG2q1O4A';
...
export default function EventDetailPage() {
  ...
  const [tenantDetails, setTenantDetails] = useState<{ org?: { footerInfo?: string } } | null>(null);
  ...
  const getEventPrice = (e: Event): { price: number; currency: string } | null => {
    if (!e.memberships?.length) return null;
    const preferences = ['recurring', 'unlimited'];
    for (const pref of preferences) {
      const item = e.memberships.find((m: EventMembership) => m.membershipType === pref);
      if (item)
        return {
          price: item.price,
  ...
  const getLocationText = (e: Event) => {
    ...
    if (e.roomName && (e.city || e.state || e.zip)) {
      const parts = [e.roomName, e.city, e.state, e.zip || e.zipCode].filter(Boolean);
      return parts.join(', ');
    }
    if (e.city) {
  ...
  const getMapsSearchUrl = (e: Event) => {
    const loc = [e.roomName, e.location, e.city, e.state, e.zip].filter(Boolean).join(', ');
    return loc ? `https://www.google.com/maps/search/${encodeURIComponent(loc)}` : null;
  };
  ...
  const handleRegister = () => {
    ...
    if (payment.type === 'paid' && event.memberships?.[0]) {
      ...
      navigate(`/checkout/event/${event.eventUrl || event.guId}/${event.memberships[0].guId}/${scheduleParam}`);
      return;
    }
  };
  ...
  const formatScheduleTime = (startTs: number, endTs: number) => {
    const s = new Date(startTs * 1000).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
    const e = new Date(endTs * 1000).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
    return `${s} - ${e}`;
  };
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg text-gray-600 dark:text-gray-300">Loading event...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-white dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center max-w-md px-4">
          <p className="text-lg text-gray-600 dark:text-gray-300 mb-6">{error || 'Event not found.'}</p>
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-4 sm:p-6 lg:p-12">
            <div className="max-w-3xl w-full min-w-0 space-y-3 sm:space-y-6">
              <div className="flex gap-2 sm:gap-3 flex-wrap">
                ...
                {(event.category?.name || (event.tagList && event.tagList[0])) && (
              ...
              <div className="inline-flex items-center gap-2 sm:gap-3 bg-black/40 hover:bg-black/50 transition-colors backdrop-blur-xl p-1.5 pr-3 sm:pr-5 rounded-full border border-white/20 shadow-xl max-w-full min-w-0">
                <div
                  ...
                  style={{ backgroundImage: `url(${HOST_PLACEHOLDER_IMAGE})` }}
                />
                <div className="flex flex-col min-w-0 flex-1">
                  ...
                  <span className="text-white text-xs sm:text-sm font-semibold truncate sm:truncate-none">
                    ...
                    {event.wemail?.designation && ` • ${event.wemail.designation}`}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4 text-lg text-gray-600 dark:text-gray-300 leading-relaxed">
                {event.template ? (
                  ...
                  <p className="whitespace-pre-wrap break-words text-gray-600 dark:text-gray-300">
                    {event.longDescription || event.shortDescription || 'No description available.'}
                  </p>
                )}
              </div>
            </section>
              ...
              <section className="min-w-0">
                ...
                <div className="overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm hover:shadow-lg transition-shadow">
                  <div className="divide-y divide-gray-200 dark:divide-gray-700">
                    {scheduleItems.map((item: { startTime: number; endTime: number; agenda?: string }, i: number) => (
                      <div
                        key={i}
                      ...
                      >
                        <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {item.agenda && (
                              <p className="text-gray-600 dark:text-gray-300 break-words">{item.agenda}</p>
                            )}
                          </div>
                        </div>
                      </div>
                ...
                <div className="flex flex-col md:flex-row gap-6 items-start">
                  ...
                  <div className="space-y-3 flex-1 min-w-0">
                    ...
                    {(() => {
                      ...
                      const aboutText = event.wemail?.description ?? orgFooterAbout;
                      return aboutText ? (
                ...
                <div className="space-y-5 mb-8">
                  {dateRangeStr && (
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-300 break-words">{dateRangeStr}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {(event.zoomMeetingUrl || event.onlineMeetJoinUrl) && (
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-bold text-gray-900 dark:text-white">Online Meet</p>
                        <p className="text-sm text-gray-600 dark:text-gray-300">
                          {event.onlineMeetProviderName === 'zoom' ? 'Zoom' : event.onlineMeetProviderName === 'googlemeet' ? 'Google Meet' : 'Other'} Platform
                        </p>
                      </div>
                    </div>
                  )}
                ...
                {!event.isPastEvent && (
                  <>
                    ...
                    <p className="text-center text-gray-600 dark:text-gray-300 text-sm mt-4 px-4">
                      Registration may include access to recording and resources.
                    </p>
                  </>
                )}
              </div>
