...
export default function EventsListPage() {
  ...
  useEffect(() => {
    const loadEvents = async () => {
      try {
        ...
        const eventList = await eventService.getPublicEvents(
          orgGuId.toString(),
        );
        ...
        const processedEvents: Event[] = eventList.map((event: any) => {
          ...
          const imagePrefixPath = `https://res.cloudinary.com/${tenant.cloudName}/image/upload/`;

          if (
            event.imageUrl &&
            !event.imageUrl.startsWith("https://res.cloudinary.com/")
          ) {
  ...
  const handleEventClick = (event: Event) => {
    // API expects event URL slug (e.g. one-start-21600807), not guId - use eventUrl when available
    const eventUrl = event.eventUrl ?? event.scheduleList?.[0]?.eventUrl;
    if (eventUrl) {
      navigate(`/events/${eventUrl}`);
    } else if (event.guId) {
  ...
  const renderPaymentBadge = (event: Event) => {
    ...
    return (
      <span
        className={`inline-flex items-center justify-center h-6 px-2 ${paymentInfo.colorClass} text-xs font-semibold rounded-full`}
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center">
            ...
            <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
              Discover exciting events and join us for memorable experiences.
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {filteredEvents.map((event) => {
                ...
                return (
                  ...
                  >
                    {/* Event Image - 3:2 aspect ratio (landscape) */}
                    <div
                      className="relative bg-gradient-to-br from-primary-100 to-brand-100 dark:from-primary-900/30 dark:to-brand-900/30 overflow-hidden"
                    ...
                    >
                      ...
                      {/* Online meeting icon (blue circle) + Payment Type Badge */}
                    ...
                    {/* Event Content – flex so short description sticks to bottom */}
                    <div className="py-4 px-6 flex flex-col flex-grow min-h-0">
                      <div className="flex-1 min-h-0 flex flex-col">
                        ...
                        {/* Date and Time */}
                        {dateTime && (
                          <div className="flex items-start gap-2 text-gray-600 mb-1 dark:text-gray-300">
                      ...
                      {/* Description – sticks to bottom of card */}
                      {(event.shortDescription) && (
                        <p className="text-gray-600 dark:text-gray-300 line-clamp-2 mt-2 overflow-hidden pt-2 border-t border-gray-100 dark:border-gray-700">
                          {event.shortDescription}
                        </p>
                      )}
                    </div>
      ...
      <div className="bg-gradient-to-r from-primary-600 to-brand-600 dark:from-primary-700 dark:to-brand-700 py-12">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-lg text-primary-100 dark:text-primary-200 mb-6">
            Contact our team to learn more about upcoming events or discuss your
            event needs.
          </p>
